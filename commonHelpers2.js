import"./assets/arrow-button-39fffa09.js";import{a as u,i as N}from"./assets/vendor-8cce9181.js";let j=document.querySelector(".quote-text"),W=document.querySelector(".quote-author");u.defaults.baseURL="https://energyflow.b.goit.study/api";const R=new Date;if(localStorage.key!=="info")F("quote");else if(TimeInStorage!==R)localStorage.removeItem("info"),F("quote");else if(TimeInStorage===R){const e=JSON.parse(localStorage.getItem("info")),t=e.quote,s=e.author;j.textContent=t,W.textContent=s}async function F(e){const t={};t.date=new Date;const s=await u.get(`${e}`),{data:{quote:a,author:r}}=s;t.author=r,t.quote=a,localStorage.setItem("info",JSON.stringify(t));const o=JSON.parse(localStorage.getItem("info"));j.textContent=o.quote,W.textContent=o.author}let A;const te=document.querySelector("[data-exercise-modal-close]"),b=document.querySelector("[data-exercise-modal]"),$=document.querySelector(".exercises-modal"),_=document.querySelector(".exercises-modal-content"),D=document.querySelector(".span-exercises-modal-loader");document.querySelector("body");const f="exerciseItems";let Y;function z(){A=document.querySelectorAll("[data-exercise-modal-open]"),A.forEach(e=>{e.addEventListener("click",t=>{h.disabledScroll(),D.classList.add("exercises-modal-loader"),Y=t.currentTarget.dataset.id,_.innerHTML="",b.classList.add("is-open"),oe(),ce()})})}async function se(){return u.defaults.baseURL="https://energyflow.b.goit.study",await u.get(`/api/exercises/${Y}`)}async function oe(){try{const t=(await se()).data;ae(t),ie(t)}catch(e){ne(e)}finally{D.classList.remove("exercises-modal-loader")}}function ae({bodyPart:e,burnedCalories:t,description:s,equipmen:a,gifUrl:r,name:o,popularity:n,rating:i,target:l,time:v}){let Q=i.toFixed(1);const X=`
  ...
  `;_.insertAdjacentHTML("beforeend",X),re(Math.round(i));const ee=document.querySelector(".exercises-modal-text"),H=document.querySelector(".exercises-modal-container-text");ee.clientHeight>H.clientHeight&&H.classList.add("exercises-modal-text-scroll")}function re(e){const s=[...$.querySelectorAll(".exercises-modal-star-icon")];for(let a=0;a<e;a+=1)s[a].classList.add("selected-stars")}function ne(e){console.log(e);const t=e.name,s=e.message;N.error({position:"topRight",message:`${t}: ${s}.`})}function ie(e){const t=document.querySelector(".exercises-modal-button-favorites"),s=document.querySelector(".exercises-modal-button-remove");localStorage.getItem(f)!==null&&JSON.parse(localStorage.getItem(f)).find(n=>n._id===e._id)?(I(),s.addEventListener("click",r)):t.addEventListener("click",a),t.addEventListener("click",a),s.addEventListener("click",r);function a(){const o=JSON.parse(localStorage.getItem(f))||[];o.push(e),localStorage.setItem(f,JSON.stringify(o)),I()}function r(){const o=JSON.parse(localStorage.getItem(f)),n=o.find(i=>i._id===e._id);o.splice(o.indexOf(n),1),localStorage.setItem(f,JSON.stringify(o)),I()}}function I(){const e=$.querySelector(".exercises-modal-button-favorites"),t=$.querySelector(".exercises-modal-button-remove");e.classList.toggle("hidden-button"),t.classList.toggle("hidden-button")}function ce(){document.addEventListener("keydown",s),te.addEventListener("click",e),b.addEventListener("click",t);function e(){b.classList.remove("is-open"),h.enabledScroll()}function t(a){a.target.hasAttribute("data-exercise-modal")&&e()}function s(a){a.code==="Escape"&&(b.classList.remove("is-open"),document.removeEventListener("keydown",s),h.enabledScroll())}}const h={scrollPosition:0,disabledScroll(){h.scrollPosition=window.scrollY,document.body.style.cssText=`
      owerflow: hidden;
      position: fixed;
      top: -${h.scrollPosition}px;
      width: 100%;
      height: 100%;
  
    `,document.documentElement.style.scrollBehavior="unset"},enabledScroll(){document.body.style.cssText="",window.scroll({top:h.scrollPosition}),document.documentElement.style.scrollBehavior=""}},J="https://energyflow.b.goit.study/api/exercises",C="formValusForSearch",P=document.querySelector(".title-container"),le=document.querySelector(".placeholder-container"),m=document.querySelector(".placeholder-container"),p=document.querySelector(".training-search-form"),d=document.createElement("ul"),L=document.querySelector(".cansel-button-ex"),V=document.querySelector(".exercises-container"),y="sessionResultOfSearch",x="pastSearchParams";let de=Number(sessionStorage.getItem("activeSearchPage"))??0,c={group:"",item:"",keyWord:""};L.addEventListener("click",()=>{L.classList.add("display-none"),p.firstElementChild.value="",sessionStorage.removeItem(C)});d.classList.add("search-result-list");le.addEventListener("click",he);p.addEventListener("submit",fe);p.addEventListener("input",pe);p.querySelector('[name="exercise-name"]').value=sessionStorage.getItem(C)??"";function ue(){sessionStorage.removeItem(y),sessionStorage.removeItem(x),sessionStorage.removeItem("activeSearchPage")}function Z(){V.classList.add("styles-for-ex-search-results")}function me(){V.classList.remove("styles-for-ex-search-results")}function ge(){p.classList.add("display-none"),P.innerHTML='<h2 class="exercises-title">Exercises</h2>'}function pe(e){L.classList.remove("display-none"),sessionStorage.setItem(C,e.target.value)}async function fe(e){e.preventDefault(),L.classList.add("display-none"),c.keyWord=p.querySelector('[name="exercise-name"]').value.trim().toLowerCase().replace(/\s/g,""),p.querySelector('[name="exercise-name"]').value="",sessionStorage.removeItem(C),m.innerHTML='<p><span class="exercises-modal-loader"></span></p>';const{data:t}=await B(c,1);if(t.results.length===0){m.innerHTML="<p>Unfortunately, <span>no results</span> were found. You may want to consider other search options to find the exercise you are looking for.Our range is wide and you have the opportunity to find more options that suit your needs.</p>";return}sessionStorage.setItem(y,JSON.stringify(t)),sessionStorage.setItem(x,JSON.stringify(c)),O(t,c)}async function he(e){if(c.keyWord="",e.target.nodeName==="UL")return;let t;if(e.target.nodeName==="DIV"&&(e.target.classList.value===""||e.target.classList.value==="list-history")&&(t=e.target),e.target.nodeName==="SPAN"&&(e.target.classList.value===""||e.target.classList.value==="list-history")&&(t=e.target.parentNode),!t)return;c.group=t.lastElementChild.textContent,c.item=t.firstChild.textContent,m.innerHTML='<p><span class="exercises-modal-loader"></span></p>';const{data:s}=await B(c);if(!s.results){m.innerHTML="<p>Unfortunately, <span>no results</span> were found. You may want to consider other search options to find the exercise you are looking for.Our range is wide and you have the opportunity to find more options that suit your needs.</p>";return}sessionStorage.setItem(y,JSON.stringify(s)),sessionStorage.setItem(x,JSON.stringify(c)),O(s,c)}async function B({group:e,item:t,keyWord:s},a=1){let r=window.innerWidth<=1439?8:9;e=e.toLowerCase(),e==="body parts"&&(e=e.slice(0,e.length-1).replace(/\s/g,""));try{return s===""?await u.get(`${J}?${e.toLowerCase()}=${t.toLowerCase()}&page=${a}&limit=${r}`):await u.get(`${J}?${e.toLowerCase()}=${t.toLowerCase()}&keyword=${s}&page=${a}&limit=${r}`)}catch(o){window.alert("something goes wrong"),console.log(o)}}function K({results:e}){return e.map(({bodyPart:t,rating:s,name:a,target:r,burnedCalories:o,time:n,_id:i})=>`<li class=exercises-serch-result>
