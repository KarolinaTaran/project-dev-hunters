import"./assets/modulepreload-polyfill-3cfb730f.js";import{a as u,i as j}from"./assets/vendor-8cce9181.js";let W=document.querySelector(".quote-text"),D=document.querySelector(".quote-author");u.defaults.baseURL="https://energyflow.b.goit.study/api";const O=new Date;if(localStorage.key!=="info")H("quote");else if(TimeInStorage!==O)localStorage.removeItem("info"),H("quote");else if(TimeInStorage===O){const e=JSON.parse(localStorage.getItem("info")),t=e.quote,s=e.author;W.textContent=t,D.textContent=s}async function H(e){const t={};t.date=new Date;const s=await u.get(`${e}`),{data:{quote:a,author:r}}=s;t.author=r,t.quote=a,localStorage.setItem("info",JSON.stringify(t));const o=JSON.parse(localStorage.getItem("info"));W.textContent=o.quote,D.textContent=o.author}let A;const Z=document.querySelector("[data-exercise-modal-close]"),v=document.querySelector("[data-exercise-modal]"),$=document.querySelector(".exercises-modal"),J=document.querySelector(".exercises-modal-content"),Y=document.querySelector(".span-exercises-modal-loader"),g="exerciseItems";let _;function B(){A=document.querySelectorAll("[data-exercise-modal-open]"),A.forEach(e=>{e.addEventListener("click",t=>{Y.classList.add("exercises-modal-loader"),_=t.target.dataset.id,J.innerHTML="",v.classList.add("is-open"),G(),te()})})}async function K(){return u.defaults.baseURL="https://energyflow.b.goit.study",await u.get(`/api/exercises/${_}`)}async function G(){try{const t=(await K()).data;Q(t),ee(t)}catch(e){P(e)}finally{Y.classList.remove("exercises-modal-loader")}}function Q({bodyPart:e,burnedCalories:t,description:s,equipmen:a,gifUrl:r,name:o,popularity:i,rating:n,target:l,time:p}){let z=n.toFixed(1);const V=`
